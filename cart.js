    ...
    this.getCart = function(userId, callback) {
        ...
        */
		...
		.toArray().then(function(result) 
		{
			 ...
			 //console.log('What is the user ID here ' + userId);
			 
			 console.log('What is the user ID here ' + result[0].userId);
		     ...
		     if(result.items != null)
			 {
				 ...
				 for(var i=0; i < result.items.length; i++)
				 {
					...
					var item = {
						_id: result.items[i]._id,
						title: result.items[i].title,
						description:  result.items[i].description,
						slogan: result.items[i].slogan,
						stars: result.items[i].stars,
						category: result.items[i].category,
						img_url: result.items[i].img_url,
					 ...
					 console.log('I found a item here whats in it _id ' + item._id);
					 console.log('I found a item here whats in it title ' + item.title);
					 console.log('I found a item here whats in it description ' + item.description);
					 
					 console.log('I found a item here whats in it slogan ' + item.slogan);
					 console.log('I found a item here whats in it category ' + item.category);
					 console.log('I found a item here whats in it img_url ' + item.img_url);
					 console.log('I found a item here whats in it price ' + item.price);
					 
					 
		
					
					//check if there are any reviews to add to the item 
					 var myreview = [];
					 if(result.items[i].reviews != null)
					 {
						 
						 if(result.items[i].reviews.length > 0)
						 {
							 for(var ii=0; ii < result.items[i].reviews.length; ii++)
							 { 
								 var areview = result.items[i].reviews[ii];
								 myreview.push(areview);
								 
								 item.reviews.push(areview);
							 }
